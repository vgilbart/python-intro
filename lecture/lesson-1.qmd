---
title: "Lesson 1 - Introduction, representing, manipulating and visualising data"
...
You can see where your current python is located by running `which python3` or `which python`. 
:::
...
::: {.callout-note} 
We are switching from typing commands in a Python interpreter to typing commands in a shell terminal window (such as bash). When you see a `$` in front of a command that tells you that you are in the shell, when you see a `>>>` it tells you are in the Python interpreter.
:::

### Script mode 

To run a script, create a folder named `code` under `swc-carpentry/`, in which a file named `intro.py` contains: 
...
The shebang `#!` followed by the interpreter `/usr/bin/env python3` can be put at the beginning of the script in order to ommit calling `python3` in command-line. If you don't put it, you will have to run `python3 code/intro.py` instead of simply `./code/intro.py`.
...
- `bash: code/intro.py: No such file or directory` i.e. you are not in the right directory to run the file or your are not writing the filename correctly.
...
- `bash: code/intro.py: Permission denied` i.e. you don't have the right to execute your script.
  ...
  Solution: run `ls -l code/intro.py` and make sure you have at least `-rwx` (read, write, exectute rights) as the first 4 characters, if not run `chmod 744 code/intro.py` to change your rights.
...
For example, we can track the weight of a patient who weighs 65 kilograms by
...
![A assigning a value to a variable is analogous to assigning a sticky note to a value.](img/sticky-variable1.png){#fig-interactive width=70%}
...
Python will assign a type automatically to any numerical value given. In the example above, variable `weight_kg` has an integer value of `65`.
...
::: {.callout-note} 
Note that `1` is a integer but `'1'` and `"1"` are strings. 
::: 
...
![Assigning values to weight_kg and weight_lb.](img/sticky-variable2.png){#fig-interactive width=100%}
...
![Changing the value of weight_kg does not impact the value of weight_lb.](img/sticky-variable3.png){#fig-interactive width=100%}
...
If an option is not provided, a default value is given. In the case of the option `ndigits`, `0` is the default. The function returns a numerical value, that corresponds to the rounded value. This value, just like any other, can be stored in a variable.
...
Read the help of the `print()` function. Print several variables (e.g `print(weight_kg, weight_lb)`). Using one of the parameters, add a comma separator in between each value. 
...
with a specific parameter instead of the `...` should look like this:
...
If your line starts by `>` your are in the python interpreter, if it starts by `$` you are in the command line of a terminal.
::: 
...
![Panda DataFrame schema from [boardinfinity](https://www.boardinfinity.com/blog/create-dataframes-using-pandas/).](img/pandas-dataframe.png){#fig-interactive width=90%}
...
![Schema of Python indexes.](img/python-zero-index.png){#fig-interactive width=100%}
...
It is also possible to use negative index, `-1` will retrieve the last item, `-2` the second last item, etc. For example, to get the value of the last patient on its first day, you could use:

```{python}
print(data.iloc[-1, 0])
```

Or to get the value last 10 patient on the 1st day, you could use:
...
print(data.iloc[:10, 20].min())
print(data.iloc[:10, 20].mean())
print(data.iloc[:10, 20].max())

print(data.iloc[0:10].mean())

patient_to_keep = data.mean(axis=1) > 6
print(data[patient_to_keep])

print(data[data.median(axis=1) > 6].iloc[:, -10:])
```
...
axes1.imshow(data, aspect='auto') # aspect='auto' to avoid squishing the plot and make it use half of the figure
...
Create a figure with two subplots, the one on the left being a boxplot (`.boxplot()`) of the average inflammation, and the one on the right being a histogram (`.hist()`) of the average inflammation.
Make them one on top of one another instead of side by side. You can also set the x and y labels of the two plots to make them more informative.
...
axes1.boxplot(ave_inflammation)
axes1.set_xlabel('Across Days')
axes1.set_ylabel('Average Inflammation')

axes2.hist(ave_inflammation)
axes2.set_xlabel('with given Average Inflammation')
axes2.set_ylabel('Number of Days')
...
We have access to 12 data sets right now, and we will want to create the same plots for all of them. We could copy and paste the code we used to create the plot for the first data set, and change the name of the data variable each time, but that would be very inefficient and error-prone.
...
::: {.callout-important title="Exercise"} 
Write a loop that calculates the sum of elements in a list by adding each element and printing the final value, so feeding the loop with the list `[124, 402, 36]` shoud print `562` after summing up.
:::
...
- "noisy" datasets that somewhat agree with our collaborator's claim, but show concerning data collection issues such as sporadic missing values and even an unsuitable candidate making it into the clinical trial.
