...
TODO verify it works in vscode or other platform
...
Create an environment with pandas at version 1.5.3. You can verify the version of pandas with the following code `print(pandas.__version__)`.
...
- the expression in both samples of the genes associated with .. GO term (TODO)
...
5. Make a pie plot that separates positive-log2FoldChange genes and negative-log2FoldChange genes. (TODO)

::: {.callout collapse="true" title="Hint"}
Create a new column, assign `+` for positive-log2FoldChange and `-` for negative-log2FoldChange. Make a pie plot of this new column.
:::
...
7. Just like 6 but make it interactive with widgets so that you can change the threshold of log2FC and padj. 
...
9. Make a heatmap of the top 5 up-regulated and top 5 down-regulated genes where the color corresponds to the `norm.count.mean` column.
Make a heatmap of the genes associated with .. GO term (TODO)
